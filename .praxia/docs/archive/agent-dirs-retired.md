...
- **Last touched in February 2026, with one exception.** No pre-existing file under the
  three roots was edited after 2026-02-14. The only later commit is `c3a95c1e`
  (2026-05-22), the docs migration: it moved 211 documents *out*
  (`.praxia/docs/migration-manifest.txt`; 0 sources left behind, 0 destinations missing)
  and **also added 8 files**, committed from what its message calls "stashed pre-existing
  untracked changes": `roles/oracle.md` (an oracle subagent definition),
  `schemas/oracle_critique.json`, `stack-config.yaml` ("Auto-generated by Orbital Velocity
  Pipeline 407"), and `workflows/{conversation_summary,document-directory,oracle-critique}.md`
  plus `workflows/scripts/{concatenate_session,convo_filter}.py`. They are removed here too:
  nothing in the tree references them, and the oracle role is what the praxia plugin now
  ships as `praxia:oracle`. *(Corrected 2026-09-22: the first version of this manifest said
  nothing was added after February, which was wrong for these 8 files — found in review.)*
...
| `workflows/` | 10 | 8 | 2026-01-22; 5 files added 2026-05-22 | Workflow definitions |
| `scripts/` | 8 | 0 | 2026-01-31 | Utility scripts incl. a jules-diff-tool binary |
| other (15 entries) | 15 | 7 | ≤ 2026-02-14; `roles/`, `schemas/`, `stack-config.yaml` added 2026-05-22 | `README.md`, `orchestration.db`, `agent.db.bak_migration_004`, `schema.sql`, status JSON, single-file dirs |
