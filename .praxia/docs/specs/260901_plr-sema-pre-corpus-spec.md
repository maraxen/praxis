...
| HM-24 | tip-typestate channel-receiver bridge shape (`plr_sema.derive.receiver_state._BRIDGE_SHAPE_RE`, `self.<attr>[<name>].<method>`, added 260902 tip typestate increment §10.2.5/§10.10 Q7 — split from a single 6-pattern row per the user's decision, option B), plus the volume bridge shape and B1's for-loop-over-comprehension-output binding (increment 5 §14.4/§14.0.1, added 260903, user-approved 260904 — see §14.11) | patterns | **3** | CAPPED (3) | None known — a syntactic pattern over how PLR is written, same argument as HM-3. Fails CLOSED (a silent family collapse, not a wrong verdict) — the reason it is its own row rather than folded into HM-25. |
| HM-25 | tip-typestate front-end syntactic patterns, the other five (typestate-anchor property shape P2, channel-default idiom P3a, and the three atom productions BoolView/NullCheck-is-None/NullCheck-is-not-None, §10.2.2/§10.2.3/§10.3.1, added 260902 tip typestate increment — split from HM-24 per the user's Q7 decision), plus a sixth pattern (delegate-call literal channel binding P9, §13.5.2, added 260903 sprint-122 backlog #4946, user-approved 260903), plus P7's accessor-anchor shape and P8's zip-comprehension operand-pairing idiom (increment 5 §14.4, added 260903, user-approved 260904 — see §14.11), plus the local-binding idiom (alpha/beta, §15.3) and the 260907 amendment's EnvRef/Zip/membership-comparator productions named inside the same entry (increment 6 §15.2/§15.3/§15.8, added 260904, user-approved 260907 — see §15.12 T31), plus the §16.5 `EnvRef` path-shape table (R-HEAD/R-ATTR/R-CONST, increment 7 §16.5/§16.9, added 260909, user-approved — D4, see T43), plus the amended predicate-position clause (a complete `ir.Seq`, R-HEAD or R-ARM, decides `T`/`F` by truthiness, move-family increment §17.1.2/§17.7, added 260909, user-approved — D7 unit 11, see T51), plus the singleton typestate anchor's own absence-rule shape (`_singleton_anchor_absent`, §17.4.2, move-family increment §17.4.0 decision 6/§17.7, added 260909, user-approved — D7 unit 12, see T52) | patterns | **12** | CAPPED (12) | None known — same argument as HM-3/HM-24. Fails LOUDLY (AC-10.1–AC-10.3/AC-15.1–AC-15.3's exact-count assertions), unlike HM-24. |
| HM-26 | site-keyed semantic models of NAMED PLR function bodies (`plr_sema.check.predicate.D6_SITE_RULES`, spec 260909_plr-sema-observation-increment.md §16.1.3/§16.15 D6, added 260909, user-approved — T48 ships `:321` (`LiquidHandler._assert_resources_exist`, deck-membership), T49 ships the `(_check_args, :375)`/`(:383)` pair, D5b) | site rules | **3** | CAPPED (3) | None known — each rule is a hand-typed claim about ONE specific PLR function's own body semantics (`_assert_resources_exist`'s `Resource.get_resource`/`Resource.__eq__`, `_check_args`'s `inspect.signature`/`**kwargs` handling), not a syntactic pattern over how PLR is generically WRITTEN (HM-24/HM-25's own criterion) — modelling either general mechanism was priced and refused (§16.1.3's Q2, §16.1.1's five-production refusal) as strictly more expensive than naming the sites directly. |
...
**Total budget: 25 registry rows** (raised from 24, D6, spec 260909_plr-sema-observation-increment.md
§16.15 — see HM-26 above). Prior to that: raised from 20, round 3 — see below. **Baseline after
round 3's discovery: 21 rows** (19 from round 2 + HM-20 + HM-21). **Headroom: 3** at that pin.
