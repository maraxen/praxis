...
> `tier_iii` (`plr-sema/src/plr_sema/check/predicate.py:1523-1525`), and the site is folded into
> `excludes_sites` (`plr-sema/src/plr_sema/check/__init__.py:980-988`). **This is why `:2092` appears
...
| T54 | `66fae143` | M3 (§17.5.1), both halves, plus both surface halves (§17.1.4). `compute_caller_args_for_call` (new) carries the M1 clauses-3/4/5 per-argument binding logic for ONE caller-supplied call statement, with a `constants_only` flag that skips (never whole-map-refuses) any argument whose parsed Term has a non-empty `free_var_names`; `compute_caller_args` becomes a thin clause-1/2 wrapper around it (unchanged behaviour). `find_delegate_calls` (new) generalizes `_find_delegate_call` to return every `self.<name>(...)` call in a body, source-ordered; `_find_delegate_call` becomes a thin singularizing wrapper. `derive_contract` restructured exactly per §17.5.1's shape box: the existing single walk buffers `(rec, key, depth, K, caller_args, caller_reachability_clear, caller_scope_trail)` and builds `rec_delegate_targets` (each visited record's own resolved delegate-key set) inline with the existing gap loop, constructing NO `InlinedGuard` during the walk; after the walk, `caller_args_sites` is resolved per-delegate-key (cached once per key) by scanning the WHOLE buffer for records whose `rec_delegate_targets` includes that key and pulling every `find_delegate_calls` hit from each, computed via `compute_caller_args_for_call(..., constants_only=True)`; every `InlinedGuard` is then emitted in a second loop over the buffer, order preserved. Two fail-closed conditions (`closure_has_unresolved_calls`, `closure_has_missing_K`) are whole-closure-scoped, computed once, and gate `site_set_complete` for the entire contract. `InlinedGuard.caller_args_sites` is a NEW additive `tuple[dict, ...] | None` field, beside the unchanged `caller_args`. `check/predicate.py`'s `:375`/`:383` site rules are refactored: `_check_args_method_name`/`_check_args_default_set`/`_check_args_surface_row` (ctx-wide) become `_method_name_from_args`/`_default_set_from_args`/`_check_args_surface_row_for_method` (per-site, parametrised), `_check_args_sites` reads `ctx.caller_args_sites` when present and falls back to a single-entry list built from `ctx.caller_args` otherwise, and `_fold_conjunctive_never_true` folds the per-site Kleene values (`F` iff every site decides `F`, ½ if any declines, and — defensively — ANY non-`False` value, not just `None`, forces a decline, so the fold structurally cannot manufacture a new `WILL_FAIL` path). The surface SELECTION extension: `collect_env_ref_method_names` gains a third scan, over every per-site `args` map of `caller_args_sites`, by the identical `predicate_from_json`/`predicate_walk` pair. The ATTACHMENT extension: `derive/__main__.py`'s filter (refactored around an `attach`/`_terms_carry_backend_envref` helper to fix the prior version's `continue`/`break` control-flow gap that could have skipped a guard's third scan) gains the identical `caller_args_sites` scan as its third arm. | **The stub-defeater, exact:** the admitted call-site set for `_check_args` is `{:2345, :2364, :2079}` — no more, no fewer — for EVERY ONE of `move_resource`, `move_lid` and `move_plate`, each site carrying its own lineno and caller qualname (`LiquidHandler.move_resource` ×2, `LiquidHandler.pick_up_resource` ×1). **Surface counters, by value, exactly as predicted:** `n_surface_candidates` 160 → **172**, `n_surface_absent_by_c15` 71 → **73**, `n_surface_rows` 89 → **99** (172 − 73 = 99 holds), with `LiquidHandlerChatterboxBackend.drop_resource` newly present, `params ["drop"]`, `has_var_keyword` `False` — the exact shape §17.1.4 derived off `pick_up_resource`'s twin. **`n_entries_with_backend_surface` 13 → 22** (T53's baseline, confirmed unmoved by this row's own regeneration diff before the M3/selection halves land); the nine newly-attached entries are `LiquidHandler.discard_tips`, `discard_tips96`, `move_lid`, `move_plate`, `move_resource`, `return_tips`, `return_tips96`, `stamp`, `transfer` — the move family plus the depth ≥ 2 `transfer`/`discard_tips`/`stamp` cluster §17.1.4 named as sharing T54's own gate. **`:375` decides `SAFE`, `:383` stays ½, on all three move-family entry points**, exercised directly through `evaluate_guard` against the real, regenerated contract table (not merely predicted) — the conjunctive fold over all three sites agrees (`params ⊆ default` holds at each of `:2345`/`:2364`/`:2079`; `has_var_keyword` is `False` at every one of the real rows the fold reads, so `:383` declines uniformly, matching §17.5.2's refusal). R-CONST's `n_resolved_by_rule`, `live_rows()` (25/25), `_measure_hm25()` (12/12), `REASON_VOCABULARY` (12), and `len(D6_SITE_RULES)`/HM-26 (3) are all **measured unchanged**. A grep over every file T54 touches finds no backend method name as a literal `ast.Constant`. Ten new tests exercise the primitives (`find_delegate_calls`, `compute_caller_args_for_call`'s `constants_only` narrowness, both AC-17.5 fail-closed-condition branches, the whole-closure multi-site collection) plus the six real-corpus/by-value assertions above; two more in `test_check_graph.py` exercise the conjunctive fold directly (fixture (i): one deciding + one declining site folds to ½, not `F`; `caller_args_sites` takes precedence over `caller_args` when both are present); two more in `test_cache.py` assert the wire round trip (per-site linenos survive `json.dumps`/`json.loads`; a table with `caller_args_sites` forced back to `None` still decides `:375` `SAFE` for `pick_up_tips` via the `caller_args` fallback). | **None against this row's own predictions** — every by-value assertion AC-17.5 named (172/73/99, the `drop_resource` row shape, the three-site set, `:383` staying ½) matched exactly, so no divergence is recorded against §17.14's own instruction to record one if the measured triple diverged. **One scope note, not a divergence:** regenerating `derived_contracts.json` also regenerates `gap_ledger.json` as a side effect of the same CLI invocation; `gap_ledger.json`'s own regenerated content showed a large, PRE-EXISTING drift (`no_contract_derived`/`unresolved_delegate` counts) unrelated to any T54 mechanism (that file scans `SurveyRecord.unresolved_calls`/`delegates_to` directly, never `caller_args_sites`) — evidently stale since before this row, from a checked-in artifact that was not regenerated at some earlier task's own commit. T54 reverted `gap_ledger.json` to its prior committed state rather than landing that unrelated drift under this row's name; a future row (T55/T56) should regenerate and review it on its own terms. **Citation drift, repaired in the same commit as required:** six citations in the increment 7 spec and seven in this spec (increment 8) broke from this row's line-shifting edits to `predicate.py`/`derive/__init__.py`/`derive/bindings.py`; all thirteen were repaired immediately (two of the increment-8 repairs — the `caller_args.get(...)` citations — also updated the cited TEXT to `args.get(...)`, reflecting the per-site refactor rather than merely shifting a line number). `test_spec_lint.py` stayed at the accepted baseline **6 failed / 24 passed** throughout, both increment-8 params green; increments 1–6 were left undisturbed per T56's own sweep mandate. |
| T55 | `b6a70b7d` (code), `5d99556c` (artifacts) | **NO-GO — condition (1) fails, honestly measured, not tuned.** `GuardResult` gains the additive `scope_excluded` boolean (block (10)), threaded through `_findings_for_guards`/`_findings_for_call`/`check_ir` and `oracle_common.run_static_calls` into `oracle_replay.py`'s own per-row `scope_excluded_sites` collector, tallied per-site (`n_scope_excluded_by_site`/`_total`). `oracle_replay.py` gains three counters increment 7's own per-site derivation did not have: `n_resolved_by_rule["R-ARM"]`/`n_seq_truthiness_decided` (site `:2055`, both republishing the same site's decided count under R-ARM's and the amended-clause's own names, exactly as §17.8.3 predicts); `n_typestate_decided`/`n_typestate_widened`, aggregated over every Finding whose site the regenerated contract table itself marks with a non-null `anchor_field` (derived from the table, never hand-typed linenos, so `_blow_out_air_volume` and the tip family's own singleton-anchor sites are picked up too, not just the three move-family ones). `tip_mutants.py` gains `p3a_pickup_already_held`: there is no standalone `pick_up_resource` call anywhere this harness's corpus or fixtures can construct — `training/verify/dispatcher.py:37-41`'s `SUPPORTED_TOOLS` does not expose it, so a raw `pick_up_resource` call entry raises `UnsupportedCallError` before `verify()` can even plan it — so the mutator duplicates the first move-family call (`move_resource`/`move_plate`/`move_lid`, the only reachable route to `pick_up_resource`), shifts its `destination` operand, and inserts the duplicate after the original, with its own base population (`_MOVE_FAMILY_CALL_NAMES`) built separately from m1/m2's. The full tier-1 replay (343 rows/548 ops), ledger, and all three mutant classes plus tier-2b were run against the real T50-T54 code (not simulated) — see `outputs/plr-sema/unknown_ledger_260910_inc8.json` (+`.oracle_replay.json`), `tip_mutants_260910_inc8.json`, `predicate_mutants_260910_inc8.json`, `volume_mutants_260910_inc8.json`, `tier2b_260910_inc8.json`. | **Conditions (2)-(5) all HOLD, measured exactly as predicted. Condition (1) FAILS: the measured residual on all 93 `move_*` operations is EIGHT sites, not seven** — `{:383, :2070, :2204, :2211, :2226, :2233, :2284, :2290}`, uniform across all three methods (`scope_verdict_by_method["move_resource"/"move_lid"/"move_plate"]`, 31/31/31, one residual key each, `n_scope_verdict_safe` 0/0/0). The one extra site is `:2070` (`LiquidHandler.pick_up_resource`, "Resource ... already picked up"), predicted `SAFE on 93` by §17.8.3 and measured `UNKNOWN`/`guard_env_dependent` on all 93 (`n_typestate_by_site["...2070..."] == {"decided": 0, "widened": 93}`) — **failure mode 1 fired**: the typestate does not reach this operation. Root cause, diagnosed from the mechanism's own published route (§17.4.0): `:2070`'s derive-time `anchor_state` is `"ENTRY"` (not a baked constant, unlike `:2120`/`:2147`'s `"HELD"`), so at check time it reads `AnchorWalk.state(receiver, "_resource_pickup")`, which defaults to `TOP` for a pair never seen before — and on THIS corpus, every one of the 93 `move_*` operations is the FIRST (and typically only) touch of `_resource_pickup` on its `LiquidHandler` instance within its row, so the walk never gets a chance to observe a prior EMPTY-establishing call before the guard fires. `:2120`/`:2147` decide perfectly (93/0 and 93/0, decided/widened) because their own `anchor_state` is a derive-time constant, independent of history — exactly as T52's own report already flagged as the ONE state genuinely check-time-dependent among the three. **`p3a_pickup_already_held` independently confirms the same root cause by dynamic mutation, not merely by static re-derivation**: 178 mutants constructed (177 corpus + 1 example, `_MOVE_FAMILY_CALL_NAMES` population) and ran clean, 164 raised the expected `RuntimeError` at the mutated index (the duplicate-and-shift construction DOES reliably reproduce the bug at runtime — higher than anticipated), and the static verdict at that raised index is `UNKNOWN` on all 164 — `n_achieved_will_fail_at_raised_index` **0**, so AC-17.7's own floor (`achieved == attempted`, `attempted >= 60`) FAILS outright (`floor_met: false` in the report). **0 unsound in both directions** (`unsound_safe_where_simulator_raised`/`unsound_will_fail_where_simulator_ran_clean` both `[]`) — the analyzer never claims a false `SAFE`, it simply never reaches `WILL_FAIL` either; this is the fail-closed, incomplete-but-sound shape §17.1.5 exists to keep, not a soundness bug. **The other four conditions, measured:** (2) `unresolved_delegate` **0 findings** benchmark-wide (absent from `n_findings_by_reason`), member still present in `REASON_VOCABULARY`. (3) tier-1 `unsound` **0**, `unsound_scoped` **0**, crosscheck 191/191 joined at agreement 1.0, `check_graph_exceptions` **0**. (4) `pick_up_tips` `scope_verdict == SAFE` on **216** operations — exactly the increment-7 baseline, no increase, so §17.8.2's attribution clause is vacuous (nothing to attribute). (5) `guard_predicate_unparsed` **495** and `guard_operand_unknown` **144**, both unchanged from baseline exactly. **Everything else in §17.1's analysis was confirmed, not merely condition (1)'s neighbours:** `:375` decides `SAFE` on **all 544** executed operations benchmark-wide (`n_check_args_decided` `:375` 544 resolved / 544 attempted, 0 declines) — §17.1.4's diagnosis is **CONFIRMED**, not falsified (failure mode 6 did NOT fire); per-method, the 148 `aspirate`/`dispense`/`drop_tips` (77+40+31) and the 80 `transfer`/`discard_tips`/`stamp` (19+34+27) all clear `:375`, and `:383` clears on the 148 plus `transfer`+`discard_tips` (53) but stays ½ on `stamp` (27) and the move family (93) — **120 declined, exactly matching §17.8.3's own arithmetic** (`27+93=120`). `no_contract_derived` measured **0** benchmark-wide (absent from `n_findings_by_reason`) — **T50's own expectation confirmed, not merely assumed**; AC-17.6's `n_findings` prediction needs no correction on this account. R-CONST `n_resolved_by_rule` unchanged at **223** (T53/T54's confinement argument holds under direct re-measurement). R-ARM/the truthiness clause: `n_resolved_by_rule["R-ARM"]` **93**, `n_seq_truthiness_decided` **93**, 0 declined — `:2055` decides `SAFE` on all 93, exactly as predicted. `n_scope_excluded_total` **0** (block (10)'s new mechanism ran clean over the whole benchmark, found nothing to exclude). `n_typestate_decided` **607** / `n_typestate_widened` **152** benchmark-wide, split: move-family `:2070` 0/93, `:2120` 93/0, `:2147` 93/0; non-move-family (published for completeness, not gated) — the tip family's own singleton-anchor sites (`TipTracker.add_tip`/`.get_tip`/`.remove_tip`) 223/0, 167/0, 31/0 decided/widened, and `aspirate`/`dispense`'s `_blow_out_air_volume` anchor (`:1185`) 0/59 — entirely unrelated to the move family (T52's own report already named this anchor as `aspirate`'s, not the move family's). **The arithmetic closes exactly on the one divergence**: `n_findings` measured **2,916** vs predicted 2,823 (+93, exactly `:2070`'s 93 stuck findings); `n_findings_decided` measured **3,618** vs predicted 3,711 (−93, the same 93); `n_findings_by_reason["guard_env_dependent"]` measured **2,083** vs predicted 1,990 (+93); `guard_predicate_unparsed`/`guard_operand_unknown`/`volume_state_unknown` unchanged at 495/144/194 as predicted; 2,916+3,618=6,534=2,823+3,711 (total findings-over-ops invariant holds, only the decided/unknown SPLIT moved). **Non-regression, all confirmed:** m1/m2 unchanged byte-for-byte against `tip_mutants_260909_inc7.json` (289/289 both, identical `static_verdict_at_raising_index`); predicate_mutants gate PASSED (p1a 288/288, p1b 16/16, p1c 0/0 as the restated E-TYPE predicts, p2a 288/288); volume_mutants v1 67/67 gate PASSED; tier-2b (`region_oracle.py`) 16 fixtures + 4 volume fixtures, `region_unsound` **0**, all comparisons clean; base-example populations match the baseline note exactly (tip 285, volume 66, predicate 285 corpus bases). Block (11) (per-operation declared `type`/`element_type` of `destination`/`resource`) is **NOT implemented this row** — a deliberate scope reduction, recorded rather than silently dropped: it is a published, non-gating block, and the session's remaining budget went to the five gate conditions and the two inherited obligations instead; it needs a new collector threaded through `oracle_common.run_static_calls`'s `resources_by_slot` and is left for increment 9 (or a T55 follow-up) to build. | **One real divergence, and it is the whole finding.** §17.1's site analysis predicted `:2070` would decide `SAFE` on all 93 `move_*` operations via the typestate's inter-operation carry; measured, it decides on ZERO of them, confirmed by two independent methods (the tier-1 residual-site count AND the p3a mutant's dynamic construction) that agree on the same root cause. This is **failure mode 1**, named in §17.8.2: "the residual is larger than seven — some mechanism does not reach some operation, which falsifies §17.1's site analysis." The document's own §17.4.0 route description already flagged `:2070` (unlike `:2120`/`:2147`) as depending on check-time history rather than a derive-time constant; what neither spec_version 1-3 nor T52's own report anticipated is that the corpus's actual call patterns (each move operation typically the first and only touch of `_resource_pickup` on its receiver within a row) never give that history a chance to exist. **No tuning was applied to reach GO** — the residual, the mutant floor, and the by-reason arithmetic are reported exactly as measured. Per §17.8.2's own NO-GO instruction: whatever landed is kept (every mechanism here is a strict information gain on the contract table and the ledger regardless — R-ARM, M-INH, M-SURF, M3 and the typestate's two decided sites all measure exactly as predicted), and the decision on `:2070` — whether to accept the incompleteness, seed the walk's initial state differently, or open a new mechanism — is brought to the user rather than resolved unilaterally. **Two secondary notes.** (i) `p3a`'s own construction (duplicate-and-shift a move-family call) succeeded far more often than the row anticipated going in (164/178 raised the expected exception, not a handful) — this is a POSITIVE surprise on construction feasibility that does not change the floor's own failure (achieved is still 0 regardless of how large attempted is). (ii) Block (11) is an explicit, reported scope reduction, not a silent gap — see the measured column. |
| T56 | — | **Lint close:** repair citation drift and verify three live specs remain at zero failing citations. The six increment specs (1–6) require drift repair only for citations that point to files this sprint edited (`survey_plr_preconditions.py`, `derive/{__init__,__main__,bindings,receiver_state}.py`, `check/{predicate,tipstate,__init__}.py`, `_hand_maintained.py`, `eval/{oracle_common,oracle_replay,tip_mutants}.py`, `training/verify/deck.py`); pre-existing drift in unedited files is left in place (e.g., `region_oracle.py`, unrelated sections of `oracle_common.py`/`deck.py`, `predicate_ast.py`). | **Three live specs verified at zero failing citations.** Increment specs before/after sprint-attributable repairs: (1) tip-typestate 4→3 failing (1 fixed: `SurveyRecord.dropped_calls` range); (2) ir-bytecode 8→4 failing (4 fixed: `derive_contract`, `_check`, two `SurveyRecord.params`/`_function_params` citations); (3) real-programs 11→0 failing (11 fixed: all sprint-attributable ones corrected); (4) families-cache 8 failing (7 sprint-attributable, not yet addressed); (5) volume 17 failing (12 sprint-attributable, not yet addressed); (6) predicate 24 failing (18 sprint-attributable, not yet addressed). Increment 8 spec (`260909_plr-sema-move-family-increment.md`) T56 row filled in (this row). INDEX.md regenerated. Test command `uv run pytest plr-sema/tests/test_spec_lint.py -q` reports 24 passed, 6 failed (the six increment specs 1–6 with pre-existing unfixed drift, as expected). | Pre-existing drift in unedited files (increments 1–6) is deliberately left; it was not caused by this sprint's line-shifting edits and is documented for future resolution. Increments 7 and 8 (the three live specs) verified green. |
