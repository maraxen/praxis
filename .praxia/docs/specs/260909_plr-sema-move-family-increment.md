...
> `parse_bridge_atom` (`plr-sema/src/plr_sema/check/tipstate.py:428-439`), is used only in the
...
> >    (`plr-sema/src/plr_sema/check/tipstate.py:428-439`) and `state_fields` the singleton anchor's own
...
> >    `_null_check`'s (`plr-sema/src/plr_sema/check/tipstate.py:372-384`). No third matcher function is
> >    added.
...
nowhere by `visit_Call` (`scripts/survey_plr_preconditions.py:290-300`), while the inner
